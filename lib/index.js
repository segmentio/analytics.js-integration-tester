...
function Tester(integration, analytics) {
  if (!(this instanceof Tester)) return new Tester(integration, analytics);
  this.integration = integration;
  this.spies = [];
  this.analytics = analytics;

  // make sure no private methods have been overriden on the prototype
  for (var prop in integration) {
    var override = integration[prop];
    var parent = Base.prototype[prop];
    if (indexOf(methods, prop) > -1) continue; // public method
    if (!parent) continue;
    if (typeof override !== 'function') continue;
    assert(
      String(override) === String(parent),
      fmt('Don\'t override the "%s" method from the base integration.', prop)
    );
  }

  // initialize noop for pageview
  if (this.integration._assumesPageview) {
...
Tester.prototype.name = function(string){
  ...
  assert(
    string === name,
    fmt('Expected name to be "%s", but it was "%s".', string, name)
  );
  return this;
};
...
Tester.prototype.option = function(key, value){
  ...
  assert(has, fmt('The integration does not have an option named "%s".', key));
 ...
 * @param {Object} object
 * @param {String} method
 * @param {Function} [fn] stubbing function
 * @return {Tester}
 */

Tester.prototype.stub = function(object, method, fn){
  // TODO: implement as a robust component
  var original = object[method];
  fn = fn || function noop(){};
  var s = function stub() { return fn.apply(object, arguments) };
  s.restore = function restore() { object[method] = original; };
  s.obj = object;
  s.method = method;
  object[method] = s;
  this.spies.push(s);
...
/**
 * Call `restore` on a specific stub.
 *
 * @param {Object} object
 * @param {String} method
 * @return {Tester}
...
Tester.prototype.restore = function(object, method){
  for (var i = 0, n = this.spies.length; i < n; i++) {
    var spy = this.spies[i];
    if (object == spy.obj && method == spy.method) {
      spy.restore();
...
Tester.prototype.didNotCall = function(spy){
  assert(
    ~indexOf(this.spies, spy), 
...
Tester.prototype.didNotReturn = function(spy, value){
  assert(
    ~indexOf(this.spies, spy),
...
Tester.prototype.load = function(fn){
  ...
  this.integration.load(function(err){
    ...
    if (!integration.loaded()) {
      return fn(new Error('The integration called back from loading, but `.loaded()` returned false.'));
    }
    fn();
  });
  return this;
...
Tester.prototype.store = function(type, id, traits){
  this.analytics[type]().identify(id, traits);
  return this;
};
