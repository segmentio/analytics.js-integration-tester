...
Tester.prototype.called = function(spy){
  ...
  each(this.spies, function(s){
    if (s == spy) match = spy;
  });

  assert(match, 'You must call `.spy(object, method)` prior to calling `.called()`.');
  assert(match.called, 'Expected "' + match.name + '" to have been called.');

  if (!args.length) return this;

  assert(match.got.apply(match, args), fmt(''
